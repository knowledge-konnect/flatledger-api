...
namespace SocietyLedger.Infrastructure.Persistence.Repositories
{
    public class AdjustmentRepository : IAdjustmentRepository
    {
        ...
        public async Task<IReadOnlyList<AdjustmentLedgerEntry>> GetByFlatIdAsync(long flatId, long societyId, DateTime? startDate, DateTime? endDate)
        {
            var query = _db.adjustments
                .Where(a => a.flat_id == flatId && a.society_id == societyId && !a.is_deleted)
                .AsQueryable();
        ...
        public async Task<decimal> GetTotalAmountBeforeDateAsync(long flatId, long societyId, DateTime before) =>
            await _db.adjustments
                .Where(a => a.flat_id == flatId && a.society_id == societyId && a.created_at < before && !a.is_deleted)
                .SumAsync(a => (decimal?)a.amount) ?? 0m;

        public async Task<decimal> GetOpeningBalanceRemainingAsync(long flatId, long societyId) =>
            await _db.adjustments
